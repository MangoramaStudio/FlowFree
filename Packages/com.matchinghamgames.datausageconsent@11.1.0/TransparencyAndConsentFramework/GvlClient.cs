...
namespace Bidtellect.Tcf
{
    ...
    public class GvlClient
    {
        ...
        protected PurposeCollection ReadPurposeCollection(JObject purposes)
        {
            ...
            foreach (var item in purposes.Children<JProperty>())
            {
                collection.Add(ReadPurpose((JObject)item.Value));
            }
        ...
        protected FeatureCollection ReadFeatureCollection(JObject features)
        {
            ...
            foreach (var item in features.Children<JProperty>())
            {
                collection.Add(ReadFeature((JObject)item.Value));
            }
        ...
        protected StackCollection ReadStackCollection(JObject stacks, StackLookup lookup)
        {
            var collection = new StackCollection();

            foreach (var item in stacks.Children<JProperty>())
            {
                collection.Add(ReadStack((JObject)item.Value, lookup));
            }

            return collection;
        }

        protected Stack ReadStack(JObject element, StackLookup lookup)
        {
            var stack = new Stack();
        ...
        protected VendorCollection ReadVendorCollection(JObject vendors, VendorLookup lookup)
        {
            var collection = new VendorCollection();

            foreach (var item in vendors.Children<JProperty>())
            {
                collection.Add(ReadVendor((JObject)item.Value, lookup));
            }

            return collection;
        }

        protected Vendor ReadVendor(JObject element, VendorLookup lookup)
        {
            ...
            if (element.TryGetValue("deletedDate", out var deletedDateElement))
            {
                vendor.DeletedDate = DateTime.Parse((string)deletedDateElement);
            }

            if (element.TryGetValue("overflow", out var overflowElement))
            {
                vendor.Overflow = new Vendor.OverflowOptions
                {
                    HttpGetLimit = ReadJsonNumber((JObject)overflowElement, "httpGetLimit"),
                };
            }
        ...
        protected bool ReadJsonBoolean(JObject element, string propertyName, bool defaultValue = default)
        {
            JToken value;
            if (element.TryGetValue(propertyName, out value))
            {
        ...
        protected int ReadJsonNumber(JObject element, string propertyName, int defaultValue = default)
        {
            JToken value;
            if (element.TryGetValue(propertyName, out value))
            {
        ...
        protected string ReadJsonString(JObject element, string propertyName, string defaultValue = default)
        {
            JToken value;
            if (element.TryGetValue(propertyName, out value))
            {
        ...
        {
            return (JArray)element[propertyName];
        }

        protected int[] ReadIdArray(JObject element, string propertyName)
        {
            return element[propertyName].Children<JValue>()
                .Select(x => (int)x)
                .ToArray();
        }
        ...
        {
            return (JObject)element[propertyName];
        }
